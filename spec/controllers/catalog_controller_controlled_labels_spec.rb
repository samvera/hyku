...
RSpec.describe 'catalog rendering of controlled vocabulary labels' do
  ...
  describe 'a property declared in CatalogController' do
    ...
    context 'when the profile marks it facetable' do
      ...
      it 'hides the id facet so staff are not offered the filter twice' do
        ...
        expect(catalog.blacklight_config.facet_fields['education_level_sim'].if).to be false
      end
  ...
  # Runs once per controller instantiation against a class-level config that
  # persists, so every change it makes has to be safe to repeat.
  it 'survives load_flexible_schema running again on every request' do
    load_profile!
    4.times { catalog.load_flexible_schema }

    shown = catalog.blacklight_config.facet_fields.select { |_, f| f.if != false }

    expect(shown.keys.grep(/resource_type/)).to eq ['resource_type_label_sim']
  end
  ...
  describe 'a property and vocabulary added only to the profile' do
    ...
    before do
      vocabulary
      Hyrax.config.controlled_vocabulary_label_service.reset!
    ...
    after { Hyrax.config.controlled_vocabulary_label_service.reset! }
