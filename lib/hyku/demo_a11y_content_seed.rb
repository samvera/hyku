...
module Hyku
  ...
  #
  # Intentionally one class file: demo seed orchestration is easier to follow than many mixins.
  class DemoA11yContentSeed
    ...
    def run
      ...
      sync_demo_content_to_solr!(collection, account: account)
      write_manifest!(account, collection, primary_work)
      Rails.logger.debug { "hyku:demo_content:seed wrote #{@manifest_path}" }
    end
    ...
    def ensure_repository_objects!
      ...
      primary_work = seed_demo_work_specs(dep, collection)
    ...
    def seed_demo_work_specs(dep, collection)
      primary_work = nil
    ...
    # Dashboard path: Hyrax::WorksControllerBehavior#create_valkyrie_work (FormFactory + CreateValkyrieWork).
    def create_demo_work!(user, klass:, title:)
      work = klass.new
      ability = Ability.new(user)
      form = Hyrax::FormFactory.new.build(work, ability, nil)
      param_key = klass.model_name.param_key
      work_attrs = demo_work_creation_attributes(klass, title)
      params = ActionController::Parameters.new(
        param_key => ActionController::Parameters.new(work_attrs).permit!,
        uploaded_files: []
      )
      ...
      raise "hyku:demo_content:seed CreateValkyrieWork validation failed: #{action.form.errors.full_messages.join(', ')}" unless action.validate

      result = action.perform
      result.value_or do |failure|
        raise "hyku:demo_content:seed CreateValkyrieWork failed: #{failure.inspect}"
      end
    end

    def demo_work_creation_attributes(klass, title)
      ...
      base.merge(demo_work_seed_extra_attributes(klass).transform_keys(&:to_s))
    end
    ...
    def attach_demo_sample_files!(work, user, paths)
      ...
      with_inline_valkyrie_attachments do
        ...
        reindex_demo_file_sets(work)
      end
    end

    def with_inline_valkyrie_attachments
      previous_valkyrie = Hyrax.config.use_valkyrie?
      previous_queue = ENV["HYRAX_ACTIVE_JOB_QUEUE"]
      ...
      begin
        ...
        Hyrax.config.use_valkyrie = previous_valkyrie
        ENV["HYRAX_VALKYRIE"] = previous_valkyrie.to_s
        ENV["HYRAX_ACTIVE_JOB_QUEUE"] = previous_queue
        ActiveJob::Base.queue_adapter = previous_adapter
      end
    end

    def reindex_demo_file_sets(work)
    ...
    def sync_demo_content_to_solr!(collection, account:)
      purge_stale_demo_solr_docs!(collection, account: account)
      index_resource!(Hyrax.query_service.find_by(id: collection.id))
      DEMO_WORK_SEED_SPECS.each do |spec|
        w = find_demo_work(spec[:klass], spec[:title])
        next unless w

        index_resource!(Hyrax.query_service.find_by(id: w.id))
      end
      sub = find_demo_sub_collection(collection)
      index_resource!(Hyrax.query_service.find_by(id: sub.id)) if sub
      nested = find_demo_work(DEMO_NESTED_WORK_CLASS, DEMO_NESTED_WORK_TITLE)
      index_resource!(Hyrax.query_service.find_by(id: nested.id)) if nested
    ...
    def purge_stale_demo_solr_docs!(collection, account:)
      cname = tenant_solr_cname(account)
      return if cname.blank?

      delete_stale_solr_docs_for_demo_title!(cname, COLLECTION_TITLE, collection.id.to_s)
      DEMO_WORK_SEED_SPECS.each do |spec|
        w = find_demo_work(spec[:klass], spec[:title])
        keep_id = w&.id&.to_s
        delete_stale_solr_docs_for_demo_title!(cname, spec[:title], keep_id)
      end

      sub = find_demo_sub_collection(collection)
      delete_stale_solr_docs_for_demo_title!(cname, DEMO_SUB_COLLECTION_TITLE, sub&.id&.to_s)

      nested = find_demo_work(DEMO_NESTED_WORK_CLASS, DEMO_NESTED_WORK_TITLE)
      delete_stale_solr_docs_for_demo_title!(cname, DEMO_NESTED_WORK_TITLE, nested&.id&.to_s)
    rescue StandardError => e
      Rails.logger.warn("[hyku:demo_content:seed] Solr stale-doc purge skipped: #{e.message}")
    end

    def delete_stale_solr_docs_for_demo_title!(cname, title, keep_id)
      ...
      stale_ids.each { |id| Hyrax::SolrService.delete(id) }
    end
