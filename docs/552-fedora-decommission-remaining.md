...
| Task | Status | Notes |
|------|--------|--------|
| Remove Fedora as selectable feature | 🟡 Partial | Runtime gated by `fedora_enabled?` (PR #2952). Optional: hardcode `disable_wings = true` and remove Fedora toggles from code/locales. |
...
| Does proprietor setup require Fedora? | ✅ No | Gated by `fedora_enabled?`. |
| Are endpoints auto-created at account creation? | ✅ Gated | Only when `fedora_enabled?`; skipped when disabled. |
...
- Fedora referenced in feature flags (gates use `fedora_enabled?`; no separate feature flags found).
