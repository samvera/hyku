...
module Hyrax
  module Admin
    module AppearancesControllerDecorator
      ...
      def update
        # In order to handle the banner image, we need to explicitly set it.  That is it is not
        # something we can update by passing the as a hash parameter but instead must (for now)
        # rely on the explicit writer method.
