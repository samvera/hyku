...
RSpec.describe 'Admin can select home page theme', type: :feature, js: true, clean: true do
  ...
  context 'when a search results theme is selected' do
    ...
    it 'supports slideshow as a default search results view' do
      login_as admin
      visit '/admin/appearance'
      click_link('Themes')
      select('Slideshow view', from: 'Search Results Page Theme')
      find('body').click
      click_on('Save')
      site = Site.last
      account.sites << site
      allow_any_instance_of(ApplicationController).to receive(:current_account).and_return(account)
      expect(page).to have_content('The appearance was successfully updated')
      expect(site.search_theme).to eq('slideshow_view')
      visit '/'
      expect(page).to have_css('body.slideshow_view')
      fill_in "search-field-header", with: "llama"
      click_button "search-submit-header"
      expect(page).to have_css('a.view-type-slideshow.active')
    end
