...
RSpec.describe 'Sitemap generation', :clean_repo, singletenant: true, type: :request do
  context 'with UUIDs as identifiers' do
    ...
    let(:public_work_id) { '8d06fd24-e84c-482b-9505-06a37a34dbe2' }

    let!(:collection) { FactoryBot.create(:public_collection, id: collection_id) }
    let!(:private_work) { FactoryBot.valkyrie_create(:generic_work_resource, visibility_setting: 'restricted', id: private_work_id) }
    let!(:public_work) { FactoryBot.valkyrie_create(:generic_work_resource, visibility_setting: 'open', id: public_work_id) }
    ...
    describe 'GET /sitemap' do
      it 'includes links to sub-sitemaps' do
        ...
        expect(locs).to include(match(%r{/sitemap/#{public_work_id[0]}}))
      end
    end
    ...
    describe 'GET /sitemap/:id' do
      context 'with a work' do
        it 'generates proper Hyrax URLs for works and collections' do
          get "/sitemap/#{public_work_id[0]}"

          expect(response).to have_http_status(:success)
          expect(response.content_type).to match(%r{application/xml})
          # Should contain work URL through main_app
          expect(locs).to include(match(%r{/concern/generic_works/#{public_work_id}}))
        end
